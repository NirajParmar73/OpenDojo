<template>
  <div class="mx-auto max-w-5xl">
    ...
    <template v-else-if="data">
      ...
      <div v-else-if="tab === 'progress'" class="space-y-4">
        <UCard><template #header><div class="flex flex-wrap items-center justify-between gap-3"><div><h2 class="font-semibold">Next grading syllabus</h2><p class="mt-1 text-sm text-slate-500">{{ data.syllabus?.targetRank ? `Preparing for ${data.syllabus.targetRank.name}` : data.syllabus?.reason }}</p></div><UBadge v-if="data.syllabus?.version" :color="data.syllabus.ready ? 'success' : 'warning'" variant="subtle">{{ data.syllabus.ready ? 'Ready' : `${data.syllabus.completed}/${data.syllabus.total} ready` }}</UBadge></div></template>
          <div v-if="data.syllabus?.version" class="space-y-5"><div class="h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><div class="h-full rounded-full bg-emerald-500" :style="{ width: `${data.syllabus.total ? Math.round(data.syllabus.completed / data.syllabus.total * 100) : 0}%` }" /></div><section v-for="section in data.syllabus.sections" :key="section.id"><h3 class="mb-2 flex items-center gap-2 text-sm font-semibold">{{ section.name }} <span v-if="section.items.some((item: any) => item.inherited)" class="text-xs font-normal text-slate-400">Includes earlier belt</span></h3><div class="divide-y divide-slate-100 rounded-xl border border-slate-200 dark:divide-slate-800 dark:border-slate-800"><div v-for="item in section.items" :key="item.id" class="flex items-center justify-between gap-3 p-3"><div><p class="text-sm font-medium">{{ item.name }} <span v-if="item.inherited" class="text-xs font-normal text-slate-400">Earlier belt</span></p><p v-if="item.description" class="mt-1 text-xs text-slate-500">{{ item.description }}</p></div><UBadge :color="item.assessment?.status === 'ready' ? 'success' : 'neutral'" variant="subtle">{{ item.assessment?.status === 'ready' ? 'Ready' : 'Working on it' }}</UBadge></div></div></section></div>
          <p v-else class="py-6 text-center text-sm text-slate-500">{{ data.syllabus?.reason || 'Your next syllabus has not been published yet.' }}</p>
        </UCard>
        <UCard><template #header><div class="flex items-center justify-between"><h2 class="font-semibold">Grading history</h2><UButton size="sm" icon="i-lucide-download" :loading="downloadingPdf === 'progress'" @click="downloadPdf(`/api/students/${data.student.id}/progress-report`, `progress_${data.student.firstName}_${data.student.lastName}.pdf`, 'progress')">Download report</UButton></div></template><div v-if="data.gradings.length" class="space-y-3"><div v-for="grading in data.gradings" :key="grading.id" class="rounded-xl border border-slate-200 p-4 dark:border-slate-800"><p class="font-medium">{{ grading.beltRank?.name }} <span class="text-slate-500">{{ grading.beltRank?.level }}</span></p><p class="mt-1 text-sm text-slate-500">Awarded {{ formatDate(grading.awardedDate) }}</p><p v-if="grading.certificateNumber" class="mt-1 text-sm">Certificate: {{ grading.certificateNumber }}</p><a v-if="grading.certificateUrl" :href="grading.certificateUrl" target="_blank" class="mt-2 inline-block text-sm text-primary hover:underline">Open certificate</a></div></div><p v-else class="py-8 text-center text-slate-500">No gradings have been recorded yet.</p></UCard>
      </div>
