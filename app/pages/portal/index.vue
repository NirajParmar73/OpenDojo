    ...
    <template v-else-if="data">
      ...
      <div v-else-if="tab === 'progress'" class="space-y-4">
        <UCard><template #header><div class="flex flex-wrap items-center justify-between gap-3"><div><h2 class="font-semibold">Next grading syllabus</h2><p class="mt-1 text-sm text-slate-500">{{ data.syllabus?.targetRank ? `Preparing for ${data.syllabus.targetRank.name}` : data.syllabus?.reason }}</p></div><UBadge v-if="data.syllabus?.version" :color="data.syllabus.ready ? 'success' : 'warning'" variant="subtle">{{ data.syllabus.ready ? 'Ready' : `${data.syllabus.completed}/${data.syllabus.total} ready` }}</UBadge></div></template>
          <div v-if="data.syllabus?.version" class="space-y-5"><div class="h-2 overflow-hidden rounded-full bg-slate-100 dark:bg-slate-800"><div class="h-full rounded-full bg-emerald-500" :style="{ width: `${data.syllabus.total ? Math.round(data.syllabus.completed / data.syllabus.total * 100) : 0}%` }" /></div><section v-for="section in data.syllabus.sections" :key="section.id"><h3 class="mb-2 text-sm font-semibold">{{ section.name }}</h3><div class="divide-y divide-slate-100 rounded-xl border border-slate-200 dark:divide-slate-800 dark:border-slate-800"><div v-for="item in section.items" :key="item.id" class="flex items-center justify-between gap-3 p-3"><div><p class="text-sm font-medium">{{ item.name }}</p><p v-if="item.description" class="mt-1 text-xs text-slate-500">{{ item.description }}</p></div><UBadge :color="item.assessment?.status === 'ready' ? 'success' : 'neutral'" variant="subtle">{{ item.assessment?.status === 'ready' ? 'Ready' : 'Working on it' }}</UBadge></div></div></section></div>
