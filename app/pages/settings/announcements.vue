        ...
        <article v-for="announcement in announcements" :key="announcement.id" class="py-4">
          <div class="flex flex-wrap items-start justify-between gap-4">
            <div class="min-w-0 flex-1">
              ...
              <p class="mt-1 text-xs text-slate-500">{{ announcement.dojo?.name || 'Entire organization' }} · {{ formatDateTime(announcement.publishedAt) }}<span v-if="announcement.expiresAt"> · expires {{ formatDateTime(announcement.expiresAt) }}</span></p>
...
type Announcement = { id: number, title: string, message: string, severity: 'info' | 'success' | 'warning' | 'urgent', dojoId: number | null, publishedAt: string, expiresAt: string | null, dojo: Dojo | null }
...
function dojoIdFromAudience() { return form.audience === 'organization' ? null : Number(form.audience.split(':')[1]) }
function edit(item: Announcement) { editingId.value = item.id; Object.assign(form, { title: item.title, message: item.message, severity: item.severity, audience: item.dojoId ? `dojo:${item.dojoId}` : 'organization', publishedAt: localDateTime(new Date(item.publishedAt)), expiresAt: item.expiresAt ? localDateTime(new Date(item.expiresAt)) : '' }); window.scrollTo({ top: 0, behavior: 'smooth' }) }
async function save() {
  saving.value = true
  try {
    const body = { title: form.title, message: form.message, severity: form.severity, dojoId: dojoIdFromAudience(), publishedAt: new Date(form.publishedAt).toISOString(), expiresAt: form.expiresAt ? new Date(form.expiresAt).toISOString() : null }
