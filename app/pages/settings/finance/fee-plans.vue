          ...
          <tbody class="bg-white divide-y divide-gray-200">
            <tr v-for="plan in feePlans" :key="plan.id">
              ...
              <td class="px-4 py-4">
                ...
                <UButton color="error" variant="ghost" size="sm" @click="archivePlan(plan)">Archive</UButton>
              </td>
            </tr>
...
async function loadData() {
  try {
    ...
    toast.add({ color: 'error', title: 'Failed to load data', description: apiErrorMessage(error) })
  }
}
...
async function createFeePlan() {
  ...
  try {
    ...
    toast.add({ color: 'error', title: 'Creation failed', description: apiErrorMessage(error) })
  } finally {
...
async function updatePlan() {
  ...
  try {
    ...
    toast.add({ color: 'error', title: 'Update failed', description: apiErrorMessage(error) })
  } finally {
...
async function archivePlan(plan: any) {
  if (!confirm(`Archive ${plan.name}? Existing assignments and payment history will be preserved.`)) return
  try {
    const result = await $fetch<{ action: 'archived' | 'deleted' }>(`/api/fee-plans/${plan.id}`, { method: 'DELETE' })
    toast.add({ color: 'success', title: result.action === 'archived' ? 'Fee plan archived' : 'Unused fee plan removed' })
    await loadData()
  } catch (error: any) {
    toast.add({ color: 'error', title: 'Could not archive fee plan', description: apiErrorMessage(error) })
  }
}
