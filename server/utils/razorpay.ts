...
export async function createRazorpayOrder(plan: RazorpayPlan, billingPeriod: RazorpayBillingPeriod, organizationId: number, organizationName: string) { return razorpayRequest<RazorpayOrder>('/orders', { method: 'POST', body: JSON.stringify({ amount: getRazorpayPrice(plan, billingPeriod), currency: 'INR', receipt: `opendojos_${organizationId}_${Date.now()}`, notes: { organization_id: String(organizationId), organization_name: organizationName.slice(0, 50), plan, billing_period: billingPeriod } }) }) }
export function verifyRazorpayPaymentSignature(orderId: string, paymentId: string, signature: string) { const { razorpayKeySecret } = config(); return safeEqual(createHmac('sha256', razorpayKeySecret).update(`${orderId}|${paymentId}`).digest('hex'), signature) }
