...
export async function requireAdmissionApplication(event: any, id: number) {
  ...
  const application = await db.query.admissionApplications.findFirst({
    ...
    with: { organization: true, dojo: true, program: true, currentBeltRank: true, reviewer: { columns: { id: true, name: true } }, physicalCopyReceiver: { columns: { id: true, name: true } }, resultingStudent: true },
  }) as any
