...
type PortalAccount = { id: number, studentId: number, passwordHash: string, isActive: number, mustChangePassword: boolean, student: { firstName: string, lastName: string, organizationId: number, dojo: { name: string } | null } | null }
...
export default defineEventHandler(async (event) => {
  ...
  const session = { user: { id: account.id, name: `${account.student.firstName} ${account.student.lastName}`, role: 'student', organizationId: account.student.organizationId, studentId: account.studentId, mustChangePassword: account.mustChangePassword, organizationName: account.student.dojo?.name || 'Student portal' }, lastLoggedIn: new Date(), sessionRefreshedAt: new Date() }
