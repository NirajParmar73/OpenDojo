...
export default defineEventHandler(async (event) => {
  ...
  const tournaments = new Map<number, { id: number, name: string, level: string, venue: string | null, startDate: Date, participantIds: Set<number> }>()
  for (const item of visible) if (item.tournament) {
    const current = tournaments.get(item.tournament.id) || { id: item.tournament.id, name: item.tournament.name, level: item.tournament.level, venue: item.tournament.venue, startDate: item.tournament.startDate, participantIds: new Set<number>() }
