...
export const students = pgTable('students', (t) => ({
  ...
  email: t.text(),
  phone: t.text(),
  membershipNumber: t.text('membership_number'),
...
export const admissionForms = pgTable('admission_forms', (t) => ({
  ...
  existingRegistrationTitle: t.text('existing_registration_title').notNull().default('Existing student registration'),
  existingRegistrationIntroduction: t.text('existing_registration_introduction').notNull().default('Already train with us? Complete this form so we can add your existing membership to OpenDojos.'),
  existingRegistrationConsentText: t.text('existing_registration_consent_text').notNull().default('I confirm that I am an existing student of this organization and that the information provided is accurate.'),
  isExistingRegistrationPublished: t.boolean('is_existing_registration_published').notNull().default(false),
...
export const admissionApplications = pgTable('admission_applications', (t) => ({
  ...
  applicationType: t.text('application_type', { enum: ['new', 'existing'] }).notNull().default('new'),
  ...
  originalJoinedAt: t.timestamp('original_joined_at', { withTimezone: true }),
  currentBeltRankId: t.integer('current_belt_rank_id').references(() => beltRanks.id, { onDelete: 'set null' }),
  currentBeltAwardedAt: t.timestamp('current_belt_awarded_at', { withTimezone: true }),
  membershipNumber: t.text('membership_number'),
...
export const admissionApplicationsRelations = relations(admissionApplications, ({ one }) => ({
  ...
  currentBeltRank: one(beltRanks, { fields: [admissionApplications.currentBeltRankId], references: [beltRanks.id] }),
