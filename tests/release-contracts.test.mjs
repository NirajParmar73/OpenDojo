...
test('syllabus readiness is versioned, territory scoped, and enforced by grading APIs', async () => {
  const schema = await read('server/database/schema.ts')
  const syllabus = await read('server/utils/syllabus.ts')
  const candidateCreate = await read('server/api/grading-exams/[id]/attempts.post.ts')
  const candidateUpdate = await read('server/api/grading-exams/[id]/attempts/[attemptId].patch.ts')
  const eligibility = await read('server/api/grading-exams/eligibility.get.ts')
  const gradingEligibility = await read('server/utils/grading-eligibility.ts')
  const portal = await read('server/api/portal/me.get.ts')
  const builder = await read('app/pages/settings/syllabus.vue')

  assert.match(schema, /export const syllabusVersions/)
  assert.match(schema, /parentVersionId/)
  assert.match(schema, /studentSyllabusAssignments/)
  assert.match(schema, /student_syllabus_item_unique/)
  assert.match(syllabus, /getManageableSyllabusScopes/)
  assert.match(syllabus, /isDojoWithinHierarchyNode/)
  assert.match(syllabus, /resolvePublishedSyllabusVersion/)
  assert.match(candidateCreate, /progress\.targetRank\.id !== body\.targetBeltRankId/)
  assert.match(candidateCreate, /if \(!progress\.ready\)/)
  assert.match(candidateUpdate, /Only the student’s next belt can be selected/)
  assert.match(eligibility, /getGradingEligibility/)
  assert.match(gradingEligibility, /syllabusReady/)
  assert.match(portal, /getStudentSyllabusProgress/)
  assert.match(builder, /Include the previous belt syllabus/)
  assert.match(builder, /Syllabus published/)
})

test('eligible candidates can be bulk scheduled and retain an editable participation lifecycle', async () => {
  const schema = await read('server/database/schema.ts')
  const eligibility = await read('server/utils/grading-eligibility.ts')
  const bulkSchedule = await read('server/api/grading-exams/bulk-schedule.post.ts')
  const candidateUpdate = await read('server/api/grading-exams/[id]/attempts/[attemptId].patch.ts')
  const candidateDelete = await read('server/api/grading-exams/[id]/attempts/[attemptId].delete.ts')
  const award = await read('server/api/grading-exams/[id]/attempts/[attemptId]/award.post.ts')
  const readiness = await read('app/components/PromotionReadinessPanel.vue')
  const readinessPage = await read('app/pages/promotion-eligibility.vue')

  assert.match(schema, /'registered', 'confirmed', 'appeared', 'absent', 'withdrawn'/)
  assert.match(schema, /grading_exam_attempt_student_unique/)
  assert.match(eligibility, /Already registered for an open grading exam/)
  assert.match(bulkSchedule, /getGradingEligibility/)
  assert.match(bulkSchedule, /db\.transaction/)
  assert.match(bulkSchedule, /for \(const \[dojoId, candidates\] of byDojo\)/)
  assert.match(bulkSchedule, /attendanceStatus: 'registered'/)
  assert.match(candidateUpdate, /Mark the candidate as appeared before recording a result/)
  assert.match(candidateDelete, /Withdraw this candidate instead of deleting/)
  assert.match(award, /Only candidates marked as appeared/)
  assert.match(readiness, /Schedule eligible students/)
