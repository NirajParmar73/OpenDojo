...
test('installable apps start in the right surface and have distinct branding', async () => {
  ...
  assert.match(installButton, /Open in Chrome/)
  assert.match(installButton, /package=com\.android\.chrome/)
  assert.match(installButton, /WhatsApp\|LinkedInApp/)
...
test('guided setup keeps tuition separate from inline additional charges', async () => {
  ...
  assert.match(guide, /configured next belt/)
...
test('guided setup and getting started surface the student spreadsheet importer', async () => {
  ...
  assert.match(onboarding, /Getting Started will guide you through student import/)
...
test('syllabus readiness is versioned, territory scoped, and enforced by grading APIs', async () => {
  ...
  const migration = await read('server/api/students/[studentId]/syllabus/migrate.post.ts')
  const progressPanel = await read('app/components/StudentSyllabusPanel.vue')
  const studentPortal = await read('app/pages/portal/index.vue')
  ...
  assert.match(syllabus, /mergeSyllabusSections/)
  assert.match(syllabus, /assessments\.length === 0/)
  assert.match(syllabus, /migrateStudentSyllabusToLatest/)
  assert.match(syllabus, /reassessmentRequired/)
  ...
  assert.match(migration, /student\.syllabus_version_updated/)
  assert.match(progressPanel, /Move to latest syllabus/)
  assert.match(progressPanel, /Includes earlier belt/)
  assert.match(studentPortal, /Includes earlier belt/)
})
...
test('landing and guided setup explain the current syllabus-to-grading workflow', async () => {
  ...
  const onboarding = await read('app/pages/onboarding.vue')
  const gettingStarted = await read('app/pages/getting-started.vue')
  const help = await read('app/utils/help-content.ts')

  assert.match(landing, /Your syllabus, your way/)
  assert.match(landing, /Simple multi-dojo gradings/)
  assert.match(landing, /reports for archived students/)
  assert.match(onboarding, /a customizable syllabus/)
  assert.match(onboarding, /Promotion readiness and multi-dojo grading exam tools/)
  assert.match(gettingStarted, /Build and publish your belt syllabus/)
  assert.match(gettingStarted, /select eligible candidates from one or several dojos/)
  assert.match(gettingStarted, /historical reports live/)
  assert.match(help, /Review eligibility and run a grading/)
  assert.match(help, /Use current and archived student reports/)
  assert.match(help, /Can I change participation after candidates are registered/)
})
